...
async def roll(ctx, dice_expression='1d20'):
    try:
        # Splits the dice expression into dice and modifier (if present)
...
async def rolla(ctx, modifier='',):
    try:
        # Rolls with advantage (2d20) and calculate the total
...
async def rolld(ctx, modifier=''):
    try:
        # Rolls with disadvantage (2d20) and calculate the total
        rolls = [random.randint(1, 20) for _ in range(2)]
        total = min(rolls)
...
async def wroll(ctx, dice_expression=''):
    try:
        ...
        # Sends the roll result as a DM to the sender
